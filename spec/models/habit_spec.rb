...
RSpec.describe Habit, type: :model do
  it 'name must be less then 50 characters ' do
    habit = described_class.create(name: Faker::Alphanumeric.alpha(number: 51), habit_type: 'goal',
  ...
  it 'must have start_date' do
    habit = described_class.create(name: Faker::Alphanumeric.alpha(number: 10), habit_type: 'goal',
                                   frequency: 'daily')
  ...
  it 'start_date must be valid date format' do
    ...
    expect(habit.errors.messages[:start_date][0]).to eq('Start date must be a valid date')
  end

  it 'habit_type should default to goal' do
    habit = described_class.create(name: Faker::Alphanumeric.alpha(number: 10),
                                   frequency: 'daily', start_date: Time.now.utc)
    expect(habit.habit_type).to eq('goal')
  end

  it 'habit_type must be goal or limit' do
    habit = described_class.create(name: Faker::Alphanumeric.alpha(number: 10), habit_type: 'not limit',
                                   frequency: 'daily', start_date: Time.now.utc)
    expect(habit.errors.messages[:habit_type][0]).to eq("Must be either 'goal' or 'limit'")
  end

  it 'frequency must be a valid frequency' do
    habit = described_class.create(name: Faker::Alphanumeric.alpha(number: 10), habit_type: 'limit',
